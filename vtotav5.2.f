      PROGRAM VTOTAV
C                                                                        NGXM means max number of grids; NOUTM means max number of output grids this code can stand.
      PARAMETER(NGXM=1000,NOUTM=2048)
      CHARACTER*80 HEADER
...
C      READ(20,*,ERR=1000,END=1000) NIONS,IDUM1,IDUM2
      ...
      READ(HEADER,*,ERR=12,END=12) I,II,III,IIII,IIIII,IIIIII
C                                                                        Total number of ions
12    NIONS=I+II+III+IIII+IIIII+IIIIII
C     READ(20,*,ERR=1000,END=1000) NIONS
      READ(20,'(A)',ERR=1000,END=1000) HEADER
      WRITE(*,*) NIONS
C                                                                        Read position of the ions. RDUM1,2,3 mean x y z direction.
      DO 10 I=1,NIONS
         READ(20,*,ERR=1000,END=1000) RDUM1,RDUM2,RDUM3
   10 CONTINUE
      WRITE(*,*) 'positions read'
      READ(20,'(A)',ERR=1000,END=1000) HEADER
C                                                                        NGX,NGY,NGZ mean number of grid points in x y z.      
      READ(20,*,ERR=1000,END=1000) NGX,NGY,NGZ
      NPLWV=NGX*NGY*NGZ
C                                                                        Choose which direction to do average.
      IF (IDIR.EQ.1) NOUT=NGX
      IF (IDIR.EQ.2) NOUT=NGY
      IF (IDIR.EQ.3) NOUT=NGZ
C                                                                        Make sure the number of points is within the raange set by the code NOUTM and NGXM. NOUTM and NGXM can be modified in the beginning. 
